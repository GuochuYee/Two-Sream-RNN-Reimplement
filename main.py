...
def save_checkpoint(state, is_best):
    # if state['epoch'] > 30:
    # torch.save(state, os.path.join(model_dir, 'Epoch-%d_Test-Acc-%.4f_Model-Best.pth.tar'%(state['epoch'], state['best_acc1'])))
    # else:
    torch.save(state, os.path.join(model_dir, 'Model-Best-NEW_2.pth.tar'))
...
def main(args):
    ...
    if args.pretrained:
        ...
        checkpoint = torch.load(os.path.join(checkpoints_dir, 'Model-Best-NEW_1.pth.tar'))
        args.lr = 0.1
